...
internal abstract class SettingsBase : ISettings {
	...
	public void AddIgnoredDevice(string path) {
		if (ignoredDevicePaths.Add(path)) {
			FireSettingsChanged();
		}
	}

	public void RemoveIgnoredDevice(string path) {
		if (ignoredDevicePaths.Remove(path)) {
			FireSettingsChanged();
		}
	}
	...
	public void RegisterSettingsChangedListener(Action listener) => this.settingsChanged += listener;
