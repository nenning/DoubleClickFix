...
internal class NativeMethods : INativeMethods {
	...
	public nint CallNextHook(nint hhk, int nCode, nint wParam, nint lParam) => CallNextHookEx(hhk, nCode, wParam, lParam);
	...
	private static readonly nint headerBuffer = Marshal.AllocHGlobal(Marshal.SizeOf<RAWINPUTHEADER>());

	public bool TryGetRawInputDeviceHandle(nint hRawInput, out nint device) {
		uint size = (uint)Marshal.SizeOf<RAWINPUTHEADER>();
		uint copied = GetRawInputData(hRawInput, RID_HEADER, headerBuffer, ref size, (uint)Marshal.SizeOf<RAWINPUTHEADER>());
		if (copied == uint.MaxValue) {
			device = 0;
			return false;
		}
		device = Marshal.PtrToStructure<RAWINPUTHEADER>(headerBuffer).Device;
		return true;
	}
